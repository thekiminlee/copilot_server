...
router.route("/officehour/questions/:id").get(function (req, res) {
  officeModel.Question.find({ office_hour_id: req.params.id })
    .sort("-createdAt")
...
router.route("/question/:id/:subscriber").patch(function (req, res) {
  officeModel.Question.findByIdAndUpdate(
    req.params.id,
    {
...
router.route("/questions/:office_hour_id").delete(function (req, res) {
  officeModel.Question.deleteMany(
    { office_hour_id: req.params.office_hour_id },
    function (err, resp) {
      if (err) {
